<section class="projects section">
	<div class="projects__group" >
		<h2>Selected Projects</h2>
	</div>
	<div class="projects__group">
		<div class="projects__block">
			<figure class="projects__image">
				<img src="assets/png/royal-george.png" alt="The Royal George website">
			</figure>
			<h3 class="projects__title"><a href="https://www.theroyalgeorgetintern.co.uk/" target="_blank" rel="noopenner noreferrer">The Royal George</a></h3>
			<p class="projects__company">Agency: Journey</p>
			<p class="projects__role">Role: Front-end Developer</p>
			<p class="projects__description">A neat presentation website and booking engine integration for a countryside pub on the River Wye.</p>
		</div>
		<div class="projects__block">
			<figure class="projects__image">
				<img src="assets/png/abhotels.png" alt="AB Hotels Website">
			</figure>
			<h3 class="projects__title"><a href="https://abhotels.co.uk/" target="_blank" rel="noopenner noreferrer">AB Hotels</a></h3>
			<p class="projects__company">Agency: Journey</p>
			<p class="projects__role">Role: Front-end Developer</p>
			<p class="projects__description">A presentation site integrated with a careers board.</p>
		</div>
		<div class="projects__block">
			<figure class="projects__image">
				<img src="assets/png/nira.png" alt="Nira Caledonia Digital Marketing">
			</figure>
			<h3 class="projects__title"><a href="" target="_blank" rel="noopenner noreferrer">Nira & Shanti Resorts</a></h3>
			<p class="projects__company">Agency: Journey</p>
			<p class="projects__role"> Role: Digital Marketing</p>
			<p class="projects__description">Digital strategy for three luxury resorts in Edinburgh, The Alps, and Mauritius.</p>
		</div>
	</div>
	<div class="projects__group">
		<div class="projects__block">
			<figure class="projects__image">
				<img src="assets/png/hoteltransfer.png" alt="Hotel Transfer on Seaham Hall's website by Journey">
			</figure>
			<h3 class="projects__title"><a href="https://www.seaham-hall.co.uk/" target="_blank" rel="noopenner noreferrer">Hotel Transfer&trade;</a></h3>
			<p class="projects__company">Agency: Journey</p>
			<p class="projects__role">Role: Front-end Developer</p>
			<p class="projects__description">Co-lead a team to create conversion suite for Journey's luxury 4 & 5-star hotels.</p>
		</div>
		<div class="projects__block">
			<figure class="projects__image">
				<img src="assets/png/dellemc.png" alt="DellEMC Emails">
			</figure>
			<h3 class="projects__title"><a href="https://www.delltechnologies.com/" target="_blank" rel="noopenner noreferrer">DellEMC</a></h3>
			<p class="projects__company">Agency: Silver</p>
			<p class="projects__role"> Role: Web Developer</p>
			<p class="projects__description">Internal comms emails for multinational corporation.</p>
		</div>
		<div class="projects__block">
			<figure class="projects__image">
				<img src="assets/png/electrolube.png" alt="Electrolube website">
			</figure>
			<h3 class="projects__title"><a href="https://electrolube.com/" target="_blank" rel="noopenner noreferrer">Electrolube</a></h3>
			<p class="projects__company">Agency: Factor 3</p>
			<p class="projects__role">Role: Web Developer</p>
			<p class="projects__description">WordPress development for multinational engineering solutions company.</p>
		</div>
	</div>
</section>